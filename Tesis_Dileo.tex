...
\begin{table}[!h]
\centering\centering
\caption{\label{tab:table-charac}Valores medios, error estándar (entre paréntesis) y prueba L.S.D.(diferencia mínima significativa) de Fisher para rendimiento y sus compo-nentes.}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
...
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{4em}lllllllllll}
\toprule
...
\bottomrule
\multicolumn{12}{l}{\textsuperscript{} *p < 0.05, **p < 0.01 and *** p < 0.001. RB: Rendimiento de algodón bruto en g, RF: Rendimiento de fibra en g, RFD: Rendimiento de}\\
\multicolumn{12}{l}{fibra al desmote en \%, PC: Peso promedio de capullos en g, NC: Número de capullos por planta, IS: Índice de semilla en g, IF:}\\
\multicolumn{12}{l}{Índice de fibra g, NSC: Número de semilla por capullo, UHML: Longitud de la fibra en mm, Str: Resistencia de la fibra en g}\\
\multicolumn{12}{l}{tex\textasciicircum{}-1\textasciicircum{}, Mic: Micronaire, IU: Índice de uniformidad de fibras \%.}\\
\end{tabular}}
\end{table}
...
\item
  Obtención de la poblacion segregante para los loci de los marcadores (M) y los QTLs. Luego de la caracterización morfológica y fisiológica, las entradas fueron seleccionadas para ser utilizadas como parentales contrastantes en los respectivos cruzamientos de este estudio. Para los mismos se utilizaron las entradas de alto porcentaje de desmote como parentales masculinos. Considerando que por cada bocha se obtiene, en promedio, 20 semillas \autocite{naeem2017} se tomó una flor de cada parental para dicho cruzamiento, para la generación de la población F\textsubscript{1} (20 plantas). La población segregante (F\textsubscript{2}) resultó de la autofecundación de las F\textsubscript{1}, para llegar a una población F\textsubscript{2} aproximada de 200 plantas \autocite{bardak2018,zhang2003}. BGSP-00166 and SP-41255 were the selected parental genotypes. Both parents have Argentine origin. The first parent is an inbreed line derived from the cross between Chaco 510 x SP 6269, while the second parent is other inbreed line selected from the cross between SP-81270-7 x Mij. 3\textbf{.} Morphologically, they differ in their plant structure (Figure \ref{fig:img-parental}). BGSP-00166 is more compact with a shorter distance between the first fruiting position and the stem, SP-41255 has a plant structure more open. The crossing was performed in both directions to evaluate the existence of reciprocal effect. The emasculation was performed in the plants used as female parent by removing the stamens with tweezers before flower anthesis (candle shape stage), during the afternoon hours. The next day, the crossing was performed by transferring pollen from male to female parents (emasculated flowers) as described in \textcite{acquaah2012}. Then F\textsubscript{1} seeds from the cotton crosses were sown under greenhouse conditions and self-pollinated to produce F\textsubscript{2}. In the 2019-2020 growing season, the F\textsubscript{2} population was sown in non-replicated rows under field conditions. Each row was 10 m long and spaced 1 m apart, with 50 plants per row (Figure \ref{fig:img-croquis}). The non-segregating generations (both F\textsubscript{1,} SP-41255 x BGSP-00166 and BGSP-00166 x SP-41255, and the parental genotypes) were also sown under the same conditions. All these generations were subjected to phenotypic evaluation. In the crossbreeding direction of SP-41255 x BGSP-00166, an F\textsubscript{2} segregating population of 205 plants was achieved, while for the reverse crossbreeding direction, 234 plants were obtained. In the same trial, as shown in Figure \ref{fig:img-croquis}, 17 plants from the BGSP-00166 parent, 19 plants from the SP-41255 parent, and 11 plants from the F\textsubscript{1} (half and half generated from a cross direction and its reciprocal) were evenly sown and achieved.
\item
  Medición de la característica fenotípica controlada por los QTL. Se realizarán las mediciones de rendimiento y calidad de fibra detalladas en el apartado ¨Caracterización morfológica¨.For the experiment of the biparental cross, each plant (from both segregating and non-segregating populations) was also individually harvested at maturity stage to evaluate yield and quality-related characteristics: SCY, LY, LP, BW, BN, FL, FS, FU and MIC were measured as described above.
\item
...
\begin{figure}
\includegraphics[width=400px]{figure/chap3/croquis_campo} \caption{Description of the field trial, showing the two populations generated from the cross in the direction SP-41255 x BGSP-00166 and its reciprocal. Within the trial, plants from the parent SP-41255 (1), Filial 1 (2), and parent BGSP-00166 (3) were randomly distributed.}\label{fig:img-croquis}
\end{figure}

\subsection{Análisis estadístico}\label{anuxe1lisis-estaduxedstico-1}

For the biparental cross experiment, a Levene´s test was done to confirm homogeneous variance in non-segregating generations, while normal distribution was confirmed by the Shapiro-Wilk´s test. Fisher's L.S.D test was conducted to compare the parentals P\textsubscript{1}, P\textsubscript{2} and the F\textsubscript{1} generation. In addition, a comparison of the means of the reciprocal F\textsubscript{1} and F\textsubscript{2} generations was also performed to detect the existence of a reciprocal effect on the characteristics under study. A Chi-Square goodness of fit test was performed to detect transgressive segregation following the approach proposed by \textcite{rodriguez2005}. Thus, it was considered that the parent generation had a size equal to that of the F\textsubscript{2} generation. The expected frequencies were calculated assuming equal size between the parent generation and the F\textsubscript{2}, and extreme phenotypes were considered to be those individuals in the F\textsubscript{2} that exceeded the average of the trait in the parent generation and two standard deviations (one-tailed test, the 2.5\% of individuals in a normal distribution). Under these assumptions, when the observed proportion of extreme individuals in the F\textsubscript{2} generation exceeded this expected proportion (a significant χ\textsuperscript{2}), it was considered that the trait showed a transgressive inheritance pattern.

The broad sense heritability (\(h_{bs}^{2}\)) was estimated from the variance components of the non-segregating generations (P\textsubscript{1}, P\textsubscript{2} and F\textsubscript{1}) and the F\textsubscript{2} generation as follows \autocite{kearsey1996}:

\[ h_{bs}^{2}=\frac{\sigma^{^2}G}{\sigma^{2}Ph}\times100 \]

Where \(h_{bs}^{2}\)= broad sense heritability, \(\sigma^{2}G\) = Genotypic variance, \(\sigma^{2}Ph\) = Phenotypic variance.

Variance among plants within the F\textsubscript{1}´s and the parents P\textsubscript{1} and P\textsubscript{2} was equated to environmental variance and subtracted from the variance among F\textsubscript{2} plants to obtain estimates of genotypic variance.

\[ \sigma^{2} E = \frac{\sigma^{2}P_1 + \sigma^{2}P_2 + \sigma^{2}F_1}{3} \]

Where \(\sigma^{2}E\) = Environmental variance, \(\sigma^{2}P_1\) = Variance of parent 1, \(\sigma^{2}P_2\) = Variance of parent 2, \(\sigma^{2}F_1\) = Variance of F\textsubscript{1} generation.

\[ \sigma^{2}G = \sigma^{2}Ph - \sigma^{2}E \]

To estimate the genetic correlation among traits the methodology proposed by \textcite{kearsey1996} was used.

\section{Resultados}\label{resultados-2}

\begin{figure}
\includegraphics[width=400px]{figure/chap3/Parentales} \caption{Plant of the parental genotypes SP-41255 and BGSP-00166}\label{fig:img-parental}
\end{figure}

\subsection{Generation of variability from the biparental cross BGSP-00166 x SP-41255}\label{generation-of-variability-from-the-biparental-cross-bgsp-00166-x-sp-41255}

The parental genotypes BGSP-00166 and SP-41255 were significant different for all the evaluated traits with exception of SCY (Table \ref{tab:table-generations-two}). On the other hand, the mean values for all traits did not show significant differences between the F\textsubscript{1} and F\textsubscript{2} reciprocal (p \textgreater{} 0.05) as shown in Table S1. This means no existence the reciprocal effect in this cross for the analyzed traits. Therefore, we then decided to merge both the F\textsubscript{1} and F\textsubscript{2} reciprocal populations.
